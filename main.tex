...
\TB{Question: when is \(G/H\) a group?}
...
\bboxnote
\begin{note}[Abelian Group's Left/Right Mod are Equivalent]
    \[
...
\begin{defn}[Partition]
    Let \(X\) be a set. A \TB{partitian} of \(X\) is a
...
\bboxcoro
\begin{coro}[G Mod H forms a Partitian of G]
    ...
\end{coro}
\ebox
...
\bboxdefn
\begin{defn}[Relation]
    A \TB{relation} on a set \(X\) is a subset of
    \(X\times X\).
\end{defn}
\ebox
...
\begin{defn}[Equivalence Relation]
    ...
    \TB{equivalence relation} if
    \begin{enumerate}
        \item \TB{Reflexivity}: \(a\sim a,\ \forall a\in X\)
        \item \TB{Symmetry}: \(a\sim b\implies b\sim a,\ \forall a,b\in X\)
...
\begin{defn}[Equivalence Class]
    ...
    on \(X\), then the \TB{equivalence class} of \(x\) is
    \[
        [x]=\class{x}_\sim=\{
            y \in X : x \sim y
        \}
...
\bboxprop
\begin{prop}[Equivalence Relations are like Set Mod Sets]
...
\bboxcoro
\begin{coro}[The Equivalence Class Partitian]
...
\bboxlem
\begin{lem}[Equivalence Relation iff Partitian]
    If \(Q\st\emptyset\not\in Q\) is a partitian of \(X\), then
    the relation \((\sim)\) defined by \(x\sim y\) \TU{iff}
...
\bboxdefn
\begin{defn}[Index]
    \(H\subgroup G\), the \TB{index} of \(H\) in \(G\) is
    \[
...
``how many elements do we \TI{really} have?''
...
\bboxcoro
\begin{coro}[Prime Ordered Groups are Cyclic]
...
\bboxnote
\begin{note}[Groups of a Certain Order]\ 
    \begin{enumerate}
        \item \(\{e\}\)
        \item \(C_2\)
        \item \(C_3\)
...
\begin{defn}[Normal Subgroup]
    \(H\le G\) is a \TB{normal subgroup} if \(gH=Hg\ \forall g\in G\)
\end{defn}
\ebox

\bboxnota
\begin{nota}[Normal Subgroup]
    \(H\nsgq G\)
\end{nota}
\ebox
...
\begin{defn}[Conjugate]
    \(g,h\in G\). The \TB{conjugate} of \(h\) by \(g\) is \(ghg^{-1}\).
\end{defn}
\ebox
